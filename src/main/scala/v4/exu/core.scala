...
// ("branch resolved",                   () => b2.valid)
  ...
  //TEST: perforamnce counters
  val _datacollEnabled: Boolean = { // check config for data collection enablement
  ...
  if (boomParams.enablePerfPrint || _datacollEnabled) {
    ...
    when (brinfos.map(_.valid).reduce(_||_)) { ctr_br_resolved     := ctr_br_resolved     + 1.U }
    ...
    class BoomPerfFinalPrint extends BlackBox with HasBlackBoxInline {
      ...
      setInline("BoomPerfFinalPrint.sv",
        s"""|module BoomPerfFinalPrint(
            ...
            |    $$fdisplay(fd, "BOOMV4_PERF_FINAL cycles=%0d instret=%0d dcache_miss=%0d icache_miss=%0d dcache_release=%0d prefetch_issued=%0d br_mispredict=%0d br_resolved=%0d itlb_miss=%0d dtlb_miss=%0d l2tlb_miss=%0d",
